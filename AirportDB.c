...
int intakeData(char *fileName) {
    ...
    if (fptr == NULL) {
        ...
        char latString[20], longString[20],
            timezoneString[3];  // Stores the latitude, longitude and timezones
                                // in strings before being converted
        ...
        char noInfoString[2] = "na";  
				// Some entries have no data and have the entry "\N", these
        ...
        for (i = 0; i < numLines; i++) {
            ...
            /* If the string is "empty" (\N), it is replaced with the string
             * "na" */
...
double coordDistance(double latitudeSearch, double longitudeSearch,
    ...
    return ( distance);  // Returns the actual displacement between airports,
                    	// complicated since it calculates the distance on a sphere.
}
...
void searchFunction(char *searchType) {
    ...
    /* To decrease redundant code, this function deals with searching for all
    string searches using a series of else if statements */
    while (strcmp(userInput, "0") != 0) {
        ...
        scanf(" %[^\n]s", userInput);  	// Reads up until the enter key is pressed which can
                           				// let the user type in multiple words.

        int count = 0;
        for (i = 0; i < numLines; i++) {
            /* If the respected match is found it prints out the airport
             * information */
            if (strcmpi(userInput, airportArr[i].city) == 0 ||
                strcmpi(userInput, airportArr[i].name) == 0 ||
                strcmpi(userInput, airportArr[i].country) == 0 ||
                strcmpi(userInput, airportArr[i].IATA) == 0 ||
        ...
        if (i == numLines && found != 1) {
            if (userInput == 0) {
                searchMenu();
            } else {  // Prints the respected outputs if nothing of the searched
                      // for item is found
                if (strcmp(searchType, "name") == 0) {
...
void searchCoords() {
    ...
    double userDistance;
    char userInput[40];
    double latitudeInput, longitudeInput; 	 // latitudeInput and longitudeInput are the users
                         					// coordinates that are entered
    double latitudeCurr, longitudeCurr; 

						// latitudeCurr and longitudeCurr are the coordinates to
                        // be comapared against for each repitition of the
                        // searching for loop

    while (userInput != 0) {
        printf(
            "Please enter the latitude and longitude (decimal degrees, 0 to "
            "exit): ");
        scanf(" %[^\n]s", userInput);
        ...
        double seperation = 0;  // Holds the value for the actual displacement between the two
               					// sets of coordinates in comparison to eachother
        ...
        for (i = 0; i < numLines; i++) {
            ...
            seperation = coordDistance(latitudeInput, longitudeInput,
                                       latitudeCurr, longitudeCurr);
            // If the coordinates are within userDistance and it is not the
            // coordinates of an existing airport (0km away) print this
            // airport's information

            // Fills array of structures of nearby airports, max 400 due to
            // logic
        ...
        for (i = 0; i < found - 1; i++) {
            for (k = 0; k < found - 1 - i; k++) {
                if (nearbyAirportArr[k].distance >
                    nearbyAirportArr[k + 1].distance) {
        ...
        printf("\n****************************************\n");
        //  Prints the results in a nice format, must make the terminal screen
        //  big for this to be viewed correctly
        printf("\n");
        ...
        for (i = 0; i < found; i++) {
            printf("%40s%21s     %.0lfkm\n", nearbyAirportArr[i].name,
                   nearbyAirportArr[i].city, nearbyAirportArr[i].distance);
        }
        printf("\n");
...
void searchMenu() {
    ...
    // This function handles the search menu interface and links to all other
    // functions
    if (opened >= 1) {
        printf("\n****************************************\n");
    }
    ...
    switch (option) {
        case 1:
            searchById();
        case 2:
            searchFunction("name");  // As stated above, 1 function is used for
                                     // all string searches,
        case 3:
            searchFunction(
                "city");  // just with different arguments to differenciate them
        case 4:
            searchFunction("country");
        case 5:
            searchFunction("IATA");
        case 6:
            searchFunction("ICAO");
